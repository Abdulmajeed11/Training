...
connection.connect(function(err,res) {  
 if(err){
  ...
  console.log("Connected");  
        ...
        connection.query('SELECT * FROM student1 ', (err, customers) => {
            if (err) {
      ...
      console.log(data,"save data")
        const query = connection.query('INSERT INTO student1 set ?', user, (err, customer) => {
              if(err)
              {
              return callback(err,null)
            }
     ...
     controller.update = (data, callback) => {
      ...
      ;
     connection.query('UPDATE student1 set ? where id = ?', [newCustomer,id], (err, rows) => {
            if(err)
            {
              return callback(err,null)
            }
      ...
      const id = data.id;
      console.log(id,"id")
      connection.query('DELETE FROM student1 WHERE id = ?', [id], (err, rows) => {
       if(err){
        return callback(err,null)
       }
       else{
